function [adout,Vres_p,Vres_n] = Coarse_sar(Vip, Vin, Vref, Vcm, N, Cm_p, Cm_n, Cd1_p, Cd1_n, Cp1_p, Cp1_n, Comp_os, del_Compvn, del_ktc, Wda)
...
if Vres_p-Vres_n >= Comp_os+del_Compvn*randn(1,1)
    Vres_p =  Vres_p - 1/2^(N+1);
    Vres_n =  Vres_n + 1/2^(N+1);
else
    Vres_p =  Vres_p + 1/2^(N+1);
    Vres_n =  Vres_n - 1/2^(N+1);
% Vres_n = Vcm + (Qi_n + (Vdam_n-Vcm)*rot90(Cm_n,3))/Ctm_n;
